...
out.print("DEBUG : db connect ok!");
 ...
 String sha_before = null;
 try{
 MessageDigest sh1 = MessageDigest.getInstance("SHA-256");
sh1.update(pw_before.getBytes("UTF-8"));
byte data[] = sh1.digest();
StringBuffer sb = new StringBuffer();
for(int i=0; i< data.length; i++)
{
	sb.append(Integer.toString((data[i]&0xff)+0x100,16).substring(1));
}
sha_before = sb.toString();
}catch(Exception e)
{
	e.printStackTrace();
}
...
String sql = "select * from test2 where pw='"+sha_before+"'";
...
String sha_after = null;
 try{
 MessageDigest sh2 = MessageDigest.getInstance("SHA-256");
sh2.update(pw_after.getBytes("UTF-8"));
byte data[] = sh2.digest();
StringBuffer sb = new StringBuffer();
for(int i=0; i< data.length; i++)
{
	sb.append(Integer.toString((data[i]&0xff)+0x100,16).substring(1));
}
sha_after = sb.toString();
}catch(Exception e)
{
	e.printStackTrace();
}
	...
	try{
	String sql = "update test2 set pw='"+sha_after+"',email='"+email+"',phone='"+phone+"',addr='"+addr+"',addr2='"+addr2+"',pwq='"+pwq+"',pwa='"+pwa+"' where id='"+id+"' and pw='"+s3+"'";
	...
	out.print("<p> sql update ok!");
		...
		session.setAttribute("sha",sha_after);
